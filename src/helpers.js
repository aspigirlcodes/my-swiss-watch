...
function getStrap(fillStyle) {
  return [
    {
    x: - variableRadius/2,
    y: -variableRadius,
    width: variableRadius,
    height: 2*variableRadius, 
    fillStyle
    }
  ]
}

function drawStrap(ctx, straps) {
  straps.forEach(strap => {
    ctx.fillStyle = strap.fillStyle;
    ctx.beginPath();
    ctx.rect(strap.x, strap.y, strap.width, strap.height);
    ctx.fill();
    ctx.stroke();
  });
}
